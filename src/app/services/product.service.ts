...
@Injectable({
  providedIn: 'root' // équivalent de le déclarer sur "providers" dans app-modules.ts
})
export class ProductService {
  ...
  public getProducts(keyword : string, page: number = 1, size: number = 4) {
    return this.http.get(`${this.host}/products?name_like=${keyword}&_page=${page}&_limit=${size}`,
      {observe: 'response'})
  }
